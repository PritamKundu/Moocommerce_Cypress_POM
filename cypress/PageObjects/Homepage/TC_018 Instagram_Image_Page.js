...
export class Instagram {
  ...
  verifyInstagraSubTitle(act_instagram_sub_title) {
    const instagram_sub_title = '//p[normalize-space()="' + act_instagram_sub_title + '"]';
    cy.xpath(instagram_sub_title).scrollIntoView().should("be.visible");
    cy.xpath(instagram_sub_title).should("have.css", "color", "rgb(75, 79, 88)");
    cy.xpath(instagram_sub_title)
      .invoke("text")
      .then((exp_instagram_sub_title) => {
        expect(exp_instagram_sub_title).to.eq(act_instagram_sub_title);
      });
  }
}
