...
export class ProductCheck {
    ...
    verifyNewsTitle(){
            ...
            cy.xpath(news).invoke('text').then((exp_news_text) => {
                expect(exp_news_text).to.eq('Nyheter')
            })
    }

    verifyProductImage(product_number){
        const product_image = '(//a[@class="woocommerce-LoopProduct-link woocommerce-loop-product__link"])['+product_number+']';
    ...
    verifyProductTitle(product_number, act_product_title){
        const product_title = '(//h2[@class="woocommerce-loop-product__title"])['+product_number+']';
            cy.xpath(product_title).should('be.visible')
            cy.xpath(product_title).invoke('text').then((exp_product_title) => {
    ...
    verifySelectOption(product_number){
        const select_option = '(//a[@class="button product_type_variable add_to_cart_button "])['+product_number+']';
            cy.xpath(select_option).should('be.visible')
            cy.xpath(select_option).invoke('text').then((exp_select_option) =>{
                expect(exp_select_option).to.eq('Velg alternativ')
            })
            cy.xpath(select_option).should('have.css', 'border-color', 'rgb(229, 171, 171)')
            cy.wait(1000)
            cy.xpath(select_option).should('have.css', 'color', 'rgb(0, 0, 0)')
            cy.wait(1000)
            cy.xpath(select_option).should('have.css', 'background-color', 'rgb(229, 171, 171)')
            cy.wait(1000)
            cy.xpath(select_option).realHover().should('have.css', 'border-color', 'rgb(229, 171, 171)')
            cy.wait(1000)
            cy.xpath(select_option).realHover().should('have.css', 'color', 'rgb(0, 0, 0)')
            cy.wait(1000)
            cy.xpath(select_option).realHover().should('have.css', 'background-color', 'rgb(229, 171, 171)')
            cy.wait(1000)
    ...
    verifyAddToWishlist(product_number) {
        const add_to_wishlist = "(//a[contains(@aria-label,'Add to Wishlist')])["+product_number+"]";
            cy.xpath(add_to_wishlist).should('be.visible')
            cy.xpath(add_to_wishlist).invoke('text').then((exp_wishlist_text) =>{
            ...
            cy.xpath(view_wishlist).should('have.css', 'border-color', 'rgb(229, 171, 171)')
            cy.wait(1000)
            cy.xpath(view_wishlist).should('have.css', 'color', 'rgb(0, 0, 0)')
            cy.wait(1000)
            cy.xpath(view_wishlist).should('have.css', 'background-color', 'rgb(229, 171, 171)')
            ...
            cy.wait(1000)
            cy.xpath(close).should('have.css', 'background-color', 'rgb(229, 171, 171)')
