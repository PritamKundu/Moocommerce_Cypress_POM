...
/* Locators */
const puchase_button =
  '//a[@class="uagb-buttons-repeater wp-block-button__link"][1]';
...
export class Banner {
  verifyBanner(banner_number) {
    ...
    if (banner_number == 1) {
      ...
      cy.xpath(puchase_button)
        .invoke("text")
        .then((act_purchase_btn) => {
          expect(act_purchase_btn).to.eq("Kjøp Nå");
        });
      cy.wait(1000);
      cy.xpath(puchase_button).should(
        "have.css",
        "border-color",
        "rgb(229, 171, 171)"
      );
  ...
  verifyBannerHeader(helper_banner_header_number) {
    const banner_header =
      '(//h2[@class="uagb-heading-text"])[' + helper_banner_header_number + "]";
