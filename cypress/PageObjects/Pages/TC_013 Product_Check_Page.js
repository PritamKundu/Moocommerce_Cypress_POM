...
export class ProductCheck {
  ...
  verifyAddToWishlist(product_number) {
    ...
    cy.wait(2000);
    cy.xpath(add_to_wishlist).focus().click({force:true});
    cy.xpath(view_wishlist, { timeout: 10000 }).should("be.visible");
    cy.xpath(view_wishlist, { timeout: 10000 }).should(
      "have.css",
      "border-color",
      "rgb(229, 171, 171)"
    );
    cy.wait(1000);
    cy.xpath(view_wishlist, { timeout: 10000 }).should("have.css", "color", "rgb(0, 0, 0)");
    cy.wait(1000);
    cy.xpath(view_wishlist, { timeout: 10000 }).should(
      "have.css",
      "background-color",
    ...
    cy.xpath(close).click({force:true});
    cy.wait(1000);
  }
}
